...
export class MedicamentService {
    ...
    saveRequest(doc: any, type: any): Observable<any> {
        ...
        } else if (type == 'REQUEST_ADDITIONAL_DATA') {
            return this.http.post<any>('/api/medicaments/save-request-additional-data', doc, {observe: 'response'});
        } else if (type == 'LABORATORY_ANALYSIS') {
            return this.http.post<any>('/api/medicaments/save-laboratory-analysis', doc, {observe: 'response'});
        }
    }
    ...
    moveToInterrupt(doc: any): Observable<any> {
        return this.http.post<any>('/api/medicaments/move-to-interrupt', doc, {observe: 'response'});
    }
}
