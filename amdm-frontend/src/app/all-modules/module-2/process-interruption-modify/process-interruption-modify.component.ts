...
@Component({
    ...
    templateUrl: './process-interruption-modify.component.html',
    styleUrls: ['./process-interruption-modify.component.css']
})
export class ProcessInterruptionModifyComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.subscriptions.push(this.activatedRoute.params.subscribe(params => {
                this.subscriptions.push(this.requestService.getMedicamentHistory(params['id']).subscribe(data => {
                        ...
                        if (data.medicamentHistory[0] && data.medicamentHistory[0].pharmaceuticalForm) {
                            this.iForm.get('medicament.pharmaceuticalForm').setValue(data.medicamentHistory[0].pharmaceuticalForm.description);
                        }
                        if (data.medicamentHistory[0]) {
                            this.iForm.get('medicament.dose').setValue(data.medicamentHistory[0].dose);
                        }
                        if (data.medicamentHistory[0] && data.medicamentHistory[0].internationalMedicamentName) {
                            this.iForm.get('medicament.internationalMedicamentName').setValue(data.medicamentHistory[0].internationalMedicamentName.description);
                        }
    ...
    back() {
        ...
        dialogRef2.afterClosed().subscribe(result => {
            if (result) {
                this.loadingService.show();
                this.outputDocuments.forEach((item, index) => {
                    if (item.docType.category === 'NL' || item.docType.category === 'OIM') { this.outputDocuments.splice(index, 1); }
                });
                this.initialData.outputDocuments = this.outputDocuments;

                this.documents.forEach((item, index) => {
                    if (item.docType.category === 'NL' || item.docType.category === 'OIM') { this.documents.splice(index, 1); }
                });
                this.initialData.outputDocuments = this.outputDocuments;
