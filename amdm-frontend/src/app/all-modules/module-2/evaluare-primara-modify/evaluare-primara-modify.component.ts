...
export class EvaluarePrimaraModifyComponent implements OnInit, OnDestroy {
    ...
    loadMedicamentDetailsByRegNr(regNr: any) {
        this.subscriptions.push(this.medicamentService.getMedicamentByRegisterNumberFullDetails(regNr).subscribe(data => {
                ...
                this.eForm.get('medicament.authorizationHolderTo').setValue(data.medicaments[0].authorizationHolder);
    ...
    initiateMedicamentDetails(data: any) {
        ...
        this.initialData.medicamentHistory.auxiliarySubstancesHistory.forEach(t => {
            t.status = 'O';
            t.compositionNumberTo = t.compositionNumber;
        });
        this.divisions = [];
        ...
        this.subscriptions.push(this.activatedRoute.params.subscribe(params => {
                this.subscriptions.push(this.requestService.getMedicamentHistory(params['id']).subscribe(data => {
                        ...
                        if (data.medicamentHistory && data.medicamentHistory.length != 0) {
                            ...
                            this.eForm.get('medicament.transferCertificate').setValue(data.medicamentHistory[0].transferCertificate);
                            this.checkAuthorizationHolder();
                        }
                        ...
                        if (data.medicamentHistory && data.medicamentHistory.length != 0) {
                            ...
                            this.eForm.get('medicament.authorizationHolderTo').setValue(data.medicamentHistory[0].authorizationHolderTo);
        ...
        this.subscriptions.push(this.administrationService.variatonTypesJSON().subscribe(data => {
                ...
                if (dataDB.variations) {
                    ...
                    for (const v of dataDB.variations) {
                        ...
                        //this.variations.push({id: v.variation.id, code : v.variation.code, value : v.value, quantity: v.quantity, comment: v.comment});
                    }
        ...
        this.subscriptions.push(
            this.administrationService.getAllPharamceuticalFormsByTypeId(this.eForm.get('medicament.pharmaceuticalFormType').value.id).subscribe(data => {
                    ...
                    if (this.initialData.medicamentHistory && this.initialData.medicamentHistory.length != 0
                        ...
                        this.eForm.get('medicament.pharmaceuticalFormTo').setValue(this.pharmaceuticalForms.find(
                            r => r.id === this.initialData.medicamentHistory[0].pharmaceuticalFormTo.id));
                    }
                },
                error => console.log(error)
            )
    ...
    nextStep() {
        ...
        if (this.eForm.get('regnr').value.regnr) {
            modelToSubmit.medicamentPostauthorizationRegisterNr = this.eForm.get('regnr').value.regnr;
        } else {
            modelToSubmit.medicamentPostauthorizationRegisterNr = this.eForm.get('regnr').value;
        }
    ...
    editSubstance(substance: any, index: number) {
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result && result.response) {
                ...
                this.activeSubstancesTable[index].quantityTo = result.activeSubstanceQuantity;
                this.activeSubstancesTable[index].unitsOfMeasurementTo = result.activeSubstanceUnit;
                ...
                this.activeSubstancesTable[index].status = result.status == 'O' ? 'O' : 'N';
                this.fillDose();
            }
        });
    }
    ...
    fillDose() {
        if (this.activeSubstancesTable.length > 3) {
            this.eForm.get('medicament.doseTo').setValue('Combinatie');
            return;
        }
    ...
    manufacturesStr(substance: any) {
        if (substance && substance.manufactures) {
            let str = Array.prototype.map.call(substance.manufactures,
    ...
    checkTransferCertificate(value: any) {
        this.eForm.get('medicament.transferCertificate').setValue(value.checked);
        this.checkAuthorizationHolder();
    }

    checkAuthorizationHolder() {
        if (this.eForm.get('medicament.transferCertificate').value) {
            this.eForm.get('medicament.authorizationHolderTo').enable();
        } else {
            this.eForm.get('medicament.authorizationHolderTo').disable();
        }
    }
