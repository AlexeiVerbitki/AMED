...
public class LicenseController {
    ...
    @Autowired
    private LicenseAgentPharmaceutistRepository licenseAgentPharmaceutistRepository;
...
//            }
    ...
    @GetMapping(value = "/load-active-license-by-id")
    public ResponseEntity<Void> loadActiveLicenseById(@RequestParam(value = "id") Integer id) throws CustomException {
        Optional<LicensesEntity> result = licensesRepository.getActiveLicenseById(id, new Date());

        if (!result.isPresent()) {
            throw new CustomException("License is not active");
        }
...
//    @PostMapping(value = "/new-pharmaceutist")
//    public ResponseEntity<Void> newPharmaceustist(@RequestBody List<LicenseAgentPharmaceutistEntity> requests){
//        logger.debug("Add pharmaceustis" + requests);
//
//        NmEconomicAgentsEntity nmEconomicAgentsEntity = economicAgentsRepository.findById(requests.get(0).getEcAgentId()).get();
//
//        nmEconomicAgentsEntity.getAgentPharmaceutist().clear();
//        nmEconomicAgentsEntity.getAgentPharmaceutist().addAll(requests);
//
//        economicAgentsRepository.save(nmEconomicAgentsEntity);
//
