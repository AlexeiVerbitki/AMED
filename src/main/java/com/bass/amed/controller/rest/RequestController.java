		...
		if (request.getMedicaments() != null) {
			...
			for (MedicamentEntity medicament : request.getMedicaments()) {
				...
				if (medicament.getStatus().equals("F")) {
					...
					medicament.setName(medicament.getCommercialName() + " " + medicament.getPharmaceuticalForm()
					                                                                    .getCode() + " " + medicament.getDose() + " " + medicament.getDivision());
				}
			}
		}
...
//    }
	...
	public ResponseEntity<Integer> saveMedicamentPostauthorizationHistoryOnInterruption(@RequestBody RegistrationRequestsEntity request) throws
	                                                                                                                                     CustomException {
...
//        request.getDocuments().removeIf(r -> r.getId() == 2236);
	...
	private void checkInstructionsHistory(List<MedicamentEntity> medicamentEntities, MedicamentHistoryEntity medicamentHistoryEntity,
		...
		for (MedicamentDivisionHistoryEntity medicamentDivisionHistoryEntity : medicamentHistoryEntity.getDivisionHistory()) {
			boolean isExistingDivisionInMedicament = medicamentEntities.stream().anyMatch(
					e -> e.getDivision().equals(medicamentDivisionHistoryEntity.getDescription()));
			if (!isExistingDivisionInMedicament) {
				for (MedicamentInstructionsHistoryEntity medicamentInstructionsHistoryEntity : medicamentHistoryEntity.getInstructionsHistory().stream().filter(
						t -> t.getDivision().equals(medicamentDivisionHistoryEntity.getDescription())).collect(Collectors.toList())) {
	...
	private void fillRemovedInstructions(MedicamentHistoryEntity medicamentHistoryEntity, MedicamentHistoryEntity medicamentHistoryEntityForUpdate,
	                                     MedicamentEntity med) {
		for (MedicamentInstructionsEntity medicamentInstructionsEntity : med.getInstructions()) {
			boolean wasFound = false;
			for (MedicamentInstructionsHistoryEntity medicamentInstructionsHistoryEntity : medicamentHistoryEntity.getInstructionsHistory().stream().filter(
					t -> t.getDivision().equals(med.getDivision())).collect(Collectors.toList())) {
	...
	private void fillNewInstructions(MedicamentHistoryEntity medicamentHistoryEntity, MedicamentHistoryEntity medicamentHistoryEntityForUpdate,
	                                 MedicamentEntity med) {
		for (MedicamentInstructionsHistoryEntity medicamentInstructionsHistoryEntity : medicamentHistoryEntity.getInstructionsHistory().stream().filter(
				t -> t.getDivision().equals(med.getDivision())).collect(Collectors.toList())) {
			boolean wasFound = false;
	...
	private MedicamentHistoryEntity fillMedicamentHistoryDetails(Integer regNr, MedicamentHistoryEntity medicamentHistoryEntity,
	                                                             MedicamentEntity medicamentEntityForUpdate) {
	...
	private void checkActiveSubstancesModifications(MedicamentHistoryEntity medicamentHistoryEntity, MedicamentEntity medicamentEntityForUpdate,
	                                                MedicamentHistoryEntity medicamentHistoryEntityForUpdate) {
		for (MedicamentActiveSubstancesEntity medicamentActiveSubstancesEntity : medicamentEntityForUpdate.getActiveSubstances()) {
			...
			Optional<MedicamentActiveSubstancesHistoryEntity> medicamentActiveSubstancesHistoryEntityOpt = medicamentHistoryEntity.getActiveSubstancesHistory()
			                                                                                                                      .stream()
			                                                                                                                      .filter(t -> t.getActiveSubstance()
			                                                                                                                                    .getId() == medicamentActiveSubstancesEntity
					                                                                                                                                   .getActiveSubstance()
					                                                                                                                                   .getId())
			                                                                                                                      .findFirst();
		...
		for (MedicamentActiveSubstancesHistoryEntity medicamentActiveSubstancesHistoryEntity : medicamentHistoryEntity.getActiveSubstancesHistory()) {
			Optional<MedicamentActiveSubstancesEntity> medicamentActiveSubstancesEntity = medicamentEntityForUpdate.getActiveSubstances().stream().filter(
					t -> t.getActiveSubstance().getId() == medicamentActiveSubstancesHistoryEntity.getActiveSubstance().getId()).findFirst();
	...
	private void checkAuxiliarySubstancesModifications(MedicamentHistoryEntity medicamentHistoryEntity, MedicamentEntity medicamentEntityForUpdate,
	                                                   MedicamentHistoryEntity medicamentHistoryEntityForUpdate) {
		for (MedicamentAuxiliarySubstancesEntity medicamentAuxiliarySubstancesEntity : medicamentEntityForUpdate.getAuxSubstances()) {
			...
			Optional<MedicamentAuxiliarySubstancesHistoryEntity> medicamentAuxiliarySubstancesHistoryEntityOpt = medicamentHistoryEntity.getAuxiliarySubstancesHistory()
			                                                                                                                            .stream()
			                                                                                                                            .filter(t -> t.getAuxSubstance()
			                                                                                                                                          .getId() == medicamentAuxiliarySubstancesEntity
					                                                                                                                                         .getAuxSubstance()
					                                                                                                                                         .getId())
			                                                                                                                            .findFirst();
		...
		for (MedicamentAuxiliarySubstancesHistoryEntity medicamentAuxiliarySubstancesHistoryEntity : medicamentHistoryEntity.getAuxiliarySubstancesHistory()) {
			Optional<MedicamentAuxiliarySubstancesEntity> medicamentAuxiliarySubstancesEntity = medicamentEntityForUpdate.getAuxSubstances().stream().filter(
					t -> t.getAuxSubstance().getId() == medicamentAuxiliarySubstancesHistoryEntity.getAuxSubstance().getId()).findFirst();
	...
	private void checkManufacturesModifications(MedicamentHistoryEntity medicamentHistoryEntity, MedicamentEntity medicamentEntityForUpdate,
		...
		for (MedicamentManufactureEntity medicamentManufactureEntity : medicamentEntityForUpdate.getManufactures()) {
			...
			Optional<MedicamentManufactureHistoryEntity> medicamentManufactureHistoryEntityOpt = medicamentHistoryEntity.getManufacturesHistory()
			                                                                                                            .stream()
			                                                                                                            .filter(t -> t.getManufacture()
			                                                                                                                          .getId() == medicamentManufactureEntity
					                                                                                                                         .getManufacture()
					                                                                                                                         .getId())
			                                                                                                            .findFirst();
		...
		for (MedicamentManufactureHistoryEntity medicamentManufactureHistoryEntity : medicamentHistoryEntity.getManufacturesHistory()) {
			Optional<MedicamentManufactureEntity> medicamentManufactureEntity = medicamentEntityForUpdate.getManufactures().stream().filter(
					t -> t.getManufacture().getId() == medicamentManufactureHistoryEntity.getManufacture().getId()).findFirst();
	...
	private MedicamentEntity fillMedicamentDetails(Integer regNr, MedicamentHistoryEntity medicamentHistoryEntity, String division) {
		...
		medicamentEntity.setName(medicamentHistoryEntity.getCommercialNameTo() + " " + medicamentHistoryEntity.getPharmaceuticalFormTo()
		                                                                                                      .getCode() + " " + medicamentHistoryEntity.getDoseTo() + " " + division);
	...
	public ResponseEntity<List<RegistrationRequestsEntity>> getRequestsByRegistrationNumber(@RequestBody Integer registrationNumber) throws CustomException {
		Optional<List<RegistrationRequestsEntity>> registrationRequestsEntities = requestRepository.findMedicamentHistoryByRegistrationNumber(
				registrationNumber);
