...
public interface ImportAuthorizationRepository extends JpaRepository<ImportAuthorizationDetailsEntity, Integer> {
	@Query(value = "SELECT * FROM import_authorization_details m WHERE (m.code_amed OR m.medicament_id like upper(CONCAT(?1, '%'))and m.approved = ?2 and m.Import_authorization_id= ?3)",
			nativeQuery = true)
