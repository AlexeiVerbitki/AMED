...
public interface PricesManagementRepository extends JpaRepository<PricesDTO, Integer>
{
    ...
    @Query(value = "SELECT rr.id               id,\n" +
            ...
            "       rr.end_date         endDate,\n" +
            "       nmp.order_approv_date orderApprovDate,\n" +
            "       rr.current_step     currentStep,\n" +
            "       pt.description      priceType,\n" +
            "       p.folder_nr         folderNr,\n" +
            "       p.mdl_value         mdlValue,\n" +
            "       nmp.expiration_date   expirationDate\n" +
            "FROM   nm_prices nmp \n" +
            "       LEFT JOIN prices p on nmp.price_request_id = p.id\n" +
            "       LEFT JOIN registration_requests rr on rr.price_id = p.id\n" +
            ...
            "  (?7 IS NULL OR nmp.order_approv_date = ?7) AND\n" +
            "  (?8 IS NULL OR p.folder_nr = ?8) AND\n" +
            "  (?9 IS NULL OR nmp.expiration_date = ?9)"
            , nativeQuery = true)
