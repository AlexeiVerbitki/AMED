...
public interface InvoiceDetailsRepository extends JpaRepository<InvoiceDetailsEntity, Integer>
{
    ...
    @Query(value = "SELECT * from invoice_details t where ((upper(t.name) like upper(CONCAT(?1, '%'))or t.medicament_id like (CONCAT(?1, '%'))) and t.authorizations_number = ?2 and t.saved = ?3)",nativeQuery = true)
